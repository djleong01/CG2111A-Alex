...
// function to handle colour data received from arduino
...
void handleIMU(TPacket *packet)
{
	char data[65];
	printf("UART IMU PACKET");
	data[0] = NET_IMU_PACKET;
	memcpy(&data[1], packet->params, sizeof(packet->params));
	sendNetworkData(data, sizeof(data));
}
...
void handleResponse(TPacket *packet)
{
	...
	switch(packet->command)
	{
		...
		case RESP_STATUS:
			handleStatus(packet);
			break;
			//  colour of object is received from arduino
		case RESP_COLOUR:
			handleColour(packet);
			break;
		case RESP_IMU:
...
void handleCommand(void *conn, const char *buffer)
{
	...
	switch(cmd)
	{
		...
		case 'G':
			...
			uartSendPacket(&commandPacket);
			break;
			// decode colour command > uartSendPacket to Ard
		case 'o':
		case 'O':
		...
		case 'w':
		case 'W':
			commandPacket.command = COMMAND_IMU_FRONT;
			uartSendPacket(&commandPacket);
			break;
		case 'a':
		case 'A':
			commandPacket.command = COMMAND_IMU_LEFT;
			uartSendPacket(&commandPacket);
			break;
		case 's':
		case 'S':
			commandPacket.command = COMMAND_IMU_BACK;
			uartSendPacket(&commandPacket);
			break;
		case 'd':
		case 'D':
			commandPacket.command = COMMAND_IMU_RIGHT;
			uartSendPacket(&commandPacket);
			break;
		case 'i':
		case 'I':
			commandPacket.command = COMMAND_GET_IMU;
			uartSendPacket(&commandPacket);
			break;
		default:
			printf("Bad command\n");
